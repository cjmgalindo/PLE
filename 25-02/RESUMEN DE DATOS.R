...
datos %>% 
  group_by(continente) %>% 
  summarise(prom_puntos = mean(puntos)) %>% 
  arrange(desc(prom_puntos)) ##ordenar en forma descendente
...
#Tabla con totales
tabla <- cbind('Frec. Absoluta' = fi, 'Frec. Relativa' = hi, 'Frec Porcentual' = pi)
total <- colSums(tabla)
...
histograma_goles <- ggplot(datos, aes(x = goles_favor)) +
  geom_histogram(binwidth = 18, fill = "steelblue", color = "black", alpha = 0.7) +
  theme_minimal() +
...
crosstab(datos$continente,datos$campeon,
         #prop.c = TRUE,  #distribución condicional por columna
         #prop.r = TRUE,  #distrbucion condicional por fila
         prop.t = TRUE,  #distribución del total
         percent = TRUE) #%
...
sink(file = "salida1.txt", append = TRUE, split = TRUE)
cat("==========================================\n")
